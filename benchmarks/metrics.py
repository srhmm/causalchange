...
def shd(true_g: nx.DiGraph, est_g: nx.DiGraph) -> int:
    ...
    for (a, b) in (t_skel & e_skel):
        if (a, b) in t_dir or (b, a) in t_dir:
...
def n_directed_edges(G) -> int:
    return len(directed_edges(G))


def n_skeleton_edges(G) -> int:
    return len(skeleton_edges(G))

def normalized_shd(true_g, est_g) -> float:
    n = true_g.number_of_nodes()
    max_edges = n * (n - 1)
    return shd(true_g, est_g) / max_edges if max_edges > 0 else 0.0
