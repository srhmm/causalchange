...
from src.stime.search.cps_search import mdl_error_cp_search, eval_fit_partition
from src.stime.sttypes import TimeseriesScoringFunction, SubsamplingApproach
from src.stime.scoring.time_space_fitting import fit_functional_model_time_space
from src.stime.utils.util_regimes import r_partition_to_windows_T
...
def score_edge_time_space(
    ...
    # Fit a different GP in each context and same regime
        ...
        score, r_partition, contexts_per_node, regimes_per_node, gps, hist, scores = \
            eval_fit_partition(data_C, links, windows_T, max_lag, discrepancy_test_type, hist=None) #todo check discrep test
        return scores
