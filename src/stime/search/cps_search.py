...
from src.stime.scoring.discrepancy_testing import discrepancy_test_all, DiscrepancyTestType, discrepancy_test_pair, \
    _cluster_weights
from src.stime.scoring.time_space_fitting import fit_gp_time_space, subsample_autoregressive, fit_functional_model_r_c
from src.stime.sttypes import ScoringFunction, SubsamplingApproach, TimeseriesScoringFunction
from src.stime.utils.util import pval_to_map
from src.stime.utils.util_regimes import cuts_to_windows_T, windows_T_to_r_partition, regimes_map_from_constraints, pelt
...
def partition_contexts_over_regimes(
    ...
    for node in range(params['N']):
        ...
        if discrepancy_test_type == DiscrepancyTestType.KCI:
            ...
            weights = {c :  np.mean(np.array([fit_functional_model_r_c(c, r, TimeseriesScoringFunction.GP , SubsamplingApproach.HORIZONTAL, datasets, windows_T,
