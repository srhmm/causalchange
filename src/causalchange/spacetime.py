...
from src.causalchange.dag.dag_time import TimeDAG
from src.causalchange.scoring.test_cond_discrepancy import DiscrepancyTestType
from src.causalchange.search.cps_search import mdl_error_cp_search, partition_regimes, partition_contexts_over_regimes
from src.causalchange.search.dag_search import dag_tree_search
from src.causalchange.util.sttypes import CpsSearchStrategy, DAGSearchSpace, MethodType, CpsInitializationStrategy, \
    TimeseriesScoringFunction
from src.causalchange.util.util_dags import compare_adj_to_links, compare_timed_adj_to_links, links_to_is_true_edge
from src.causalchange.util.util_regimes import partition_t, r_partition_to_windows_T, windows_T_to_r_partition, \
    regimes_map_from_constraints
from src.causalchange.util.util_regimes import precision_recall_dist_cps, ari, nmi, safe_div
...
class SpaceTime:
    ...
    discrepancy_test: DiscrepancyTestType
    ...
    def __init__(
        ...
        * *to_file* (``int``) -- store outputs
        ...
        self.defaultargs = {
            ...
            "discrepancy_test": DiscrepancyTestType.KCI,
            ...
            "logger": None, "out": "", "verbosity": 0, "to_file": False, "eval": True}
        ...
        if self.method_type == MethodType.ST_GP_DAG: self.cps_init_strategy = CpsInitializationStrategy.BINS #skip
    ...
    def run(self, data: dict):
        ...
        for self.cur_it in range(self.interleave_its):
            ...
            if self.hybrid_cps_dag_discovery or self.converged:
                break
        self._info(f'Completed run in {self.n_it_completed} iterations. ')
    ...
    def dag_search(self, data: dict):
        ...
        is_true_edge = lambda i: lambda j: "" if self.truths is None else links_to_is_true_edge(self.truths.true_links)
        ...
        if self.search_space == DAGSearchSpace.GREEDY_TREE_SEARCH:
            raise DeprecationWarning("update signature")
    ...
    def cps_search(self, data: dict):
        ...
        # CPS SEARCH using residual distributions and MDL
            ...
            self.result.cur_regimes, self.result.cur_contexts, self.result.cur_c_each_node, self.result.cur_r_each_node = \
                mdl_error_cp_search(data, self.result.cur_links, self.max_lag, self.discrepancy_test, self.min_dur)
        else:
    ...
    def _set_initial_regimes(self, data: dict) -> None:
        ...
        :return:
        """
        if self.cps_strategy not in [CpsSearchStrategy.SKIP] and self.cps_init_strategy not in [CpsInitializationStrategy.HYBRID, CpsInitializationStrategy.BINS]:
            self._info('*** CPS Search ***')
        ...
        # Cps discovery under the empty graph
            ...
            found_r_partition, _, found_c_partitions, _ = (
                mdl_error_cp_search(data, self_links, self.max_lag, self.discrepancy_test,self.min_dur))
        ...
        # Cps discovery under the fully connected graph
            ...
            found_r_partition, _, n_partitions = (
                mdl_error_cp_search(data, full_links, self.max_lag, self.discrepancy_test, self.min_dur))
            self.result.initial_regimes = found_r_partition
        else:
    ...
    def get_metrics(self):
        ...
        metrics_cps = {}
        CPS_TOL = 3
        if not self.eval: return

        eval_dag_search = self.search_space != DAGSearchSpace.SKIP and self.n_it_completed > 0
        if eval_dag_search:
            assert self.truths is not None
            ...
            metrics_dag_timed = compare_timed_adj_to_links(
                ...
                self.truths.true_links, self.method_type, self.max_lag, False, self.logger,  self.verbosity)
        eval_cps_search = self.cps_strategy != CpsSearchStrategy.SKIP  and self.n_it_completed > 0
        if eval_cps_search:
            assert self.truths is not None and self.truths.regimes_partition is not None
    ...
    """ Utilities """
    def util_plot_graph(self, plot_summary_graph: bool, out_file=None):
        from tigramite.toymodels import structural_causal_processes as toys
        import tigramite.plotting as tp
        import matplotlib.pyplot as plt

        found_links = self.result.cur_links[0]
        strength_mat = self.util_edge_strength_matrix()
        gt_graph = toys.links_to_graph(found_links, tau_max=self.max_lag)
        ...
        if plot_summary_graph:
            ...
            plt.show() if out_file is None else plt.savefig(out_file)
        else:
    ...
    def util_plot_regimes(self, data):
        import src.exp.exp_stime.utils.plot_timeseries as stp
        found_regimes = self.result.cur_regimes
        stp.plot_timeseries_regimes_contexts(data, found_regimes)

    def util_edge_strength_matrix(self):
        dag_model = self.result.cur_dag_model
        _ = dag_model.get_timed_adj()
            ...
            for j in dag_model.nodes:
                for lag in range(dag_model.max_lag):
                    if dag_model.is_edge((i, lag), j):
                        ...
                        #strength_mat[i][j][lag] = dag_model.eval_edge(j, [(i, lag)])
    ...
    def util_score_given_links(self, data: dict, links: dict, windows_T: list = list(), r_partition: list = list(),
                               to_file=True):
        ...
        if len(windows_T) > 0 and len(r_partition) > 0:
            ...
            self._set_initial_regimes(data)
            windows_to_use = r_partition_to_windows_T(self.result.initial_regimes, skip=self.max_lag)
    ...
    def util_partition_under_regimes(self, data, links, windows_T, discrepancy_test):
        ...
        regimes_per_node = partition_regimes(data, links, windows_T, {'N': n_n, 'T': n_t}, discrepancy_test)
        regimes = regimes_map_from_constraints(regimes_per_node)
        r_partition = windows_T_to_r_partition(windows_T, self.max_lag, regimes)

        contexts_per_node = partition_contexts_over_regimes(data, links, regimes, windows_T, {'N': n_n, 'T': n_t}, discrepancy_test)
