...
from src.causalchange.util.util import data_scale
from src.stime.scoring.time_space_fitting import fit_gaussian_process
from src.stime.sttypes import TimeseriesScoringFunction
...
def fit_functional_model_context(
    ...
    # helper to concat a set/list of contexts and fit one GP
    ...
    # Optional: cached multiway refit (only used ONCE per final group for scoring)
    @lru_cache(maxsize=None)
    ...
    # --- 2) Pairwise joint fits and Δ matrix ---
    ...
    # --- 3) Connected components from significant negative Δ ---
    ...
    # --- 4) Final MDL over groups (single refit per group; still cheap) ---
    group_models = []
    ...
    results = {
        ...
        "labels_pred": labels_pred,        # shape (C,), aligned to `contexts`
        "contexts": contexts,              # list of context ids used
...
def fit_functional_model_context_greedy_group_merges(
    ...
    # --- 3) greedy MDL merging using Δ and k = -log2(alpha) ---
    k_bits = -np.log2(alpha) if alpha is not None else 0.0
    ...
    # --- 4) final MDL over groups + bookkeeping ---
    final_total = total_L()
    group_models = []
    for G in groups:
        ct = tuple(sorted(G))
