...
class TimeDAG(DAG):
    def __init__(self, data: dict, n_n: int, edges_state: EdgeMemoized, #max_lag: int, min_dur: int,
                 **optargs):
        r""" Timed DAG model.
        ...
        * *verbosity* (``int``) -- verbosity level
        """
        super().__init__(data, n_n, edges_state, **optargs)
        self.min_dur = optargs.get("min_dur", 30)
        self.max_lag = optargs.get("max_lag", 0) + 1  # using range for lags within TimeDag
