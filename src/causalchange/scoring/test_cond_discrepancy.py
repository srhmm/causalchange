...
from src.causalchange.util.utils import pval_to_map, is_insignificant
from src.causalchange.util.util_tests import test_mechanism
...
def discrepancy_test_all(
    ...
    if discrepancy_test_type == DiscrepancyTestType.SKIP:
        ...
        parents = [1 if node_i in pa_i else 0 for node_i in range(Dc_list[0].shape[1])]
        mp =  _cluster_mechanism_regimes(gp_time_space, ws)
    else:
        raise ValueError(discrepancy_test_type)

    return mp, pval_mat

def _cluster_mechanism_regimes(gp_time_space, ws):
    """Tests a mechanism (regimes only, all contexts taken together)"""
    weights = {r:  np.mean(np.array([gp_time_space[context][r][0] for context in gp_time_space])) for r in range(len(ws))}
    return [0] if len(weights) == 1 else _cluster_weights(weights)[0]
...
def _cluster_weights(
...
) -> Tuple[List[int], Dict[int, List[str]], KMeans, int]:
    ...
    return labels, clusters, best_model, best_k
...
def discrepancy_test_pair(
    ...
    if discrepancy_test_type == DiscrepancyTestType.SKIP:
        pass
    else: #todo check w clust needed#if discrepancy_test_type == DiscrepancyTestType.KCI:
