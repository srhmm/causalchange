...
from src.causalchange.scoring.fit_time_space import fit_gp_time_space, collect_subsample, fit_functional_model_r_c
from src.causalchange.scoring.test_cond_discrepancy import discrepancy_test_all, DiscrepancyTestType, discrepancy_test_pair, \
    _cluster_weights
from src.causalchange.util.sttypes import TimeseriesScoringFunction
from src.causalchange.util.utils import pval_to_map

from src.causalchange.util.util_regimes import cuts_to_windows_T, windows_T_to_r_partition, regimes_map_from_constraints, pelt


""" Regime Changepoint Search """


def mdl_error_cp_search(datasets: dict, links: dict, skip: int, discrepancy_test, min_dur=10, single_run=True, return_residuals=False):
    """ changept search only"""
    ...
    while cuts[-1] != params['T']:
        ...
        current_mdl, current_r_partition, current_contexts, current_regimes, current_gps, hist, _ = (
            eval_fit_partition(datasets, links, windows_T, skip, discrepancy_test))
    ...
    regimes_per_node = partition_regimes(datasets, links, windows_T, params, discrepancy_test)
    ...
    contexts_per_node = partition_contexts_over_regimes(datasets, links, regimes, windows_T, params, discrepancy_test)
...
def _arbitrary_cps(datasets, links, params, options, w_size, skip, discrepancy_test):
    windows_T = [(b + skip, b + w_size) for b in range(0, datasets.shape[1], w_size)]
    res = partition_regimes(datasets, links, windows_T, params, discrepancy_test)
    regimes = regimes_map_from_constraints(res)

    return windows_T_to_r_partition(windows_T, skip, regimes)


def iterative_cp_search(datasets, links, skip, discrepancy_test, grain=None):
    """
    ...
    while improvement and len(candidate_cps) > 0:
        ...
        for candidate in candidate_cps:
            ...
            mdl_candidate[candidate], r_partition, contexts_per_node, regimes_per_node, gps, hist, _ = (
                eval_fit_partition(datasets, links, candidate_windows_T.copy(), skip, discrepancy_test, hist))
...
def eval_fit_partition(datasets, links, windows_T, skip, discrepancy_test, hist=None):
    """
    ...
    regimes_per_node = partition_regimes(
        datasets, links, windows_T, params, discrepancy_test)  # get regime partitions per node (using KCI)
...
def partition_contexts_over_regimes(
        datasets, links, regimes, windows_T, params, discrepancy_test_type):
    """
        ...
        if discrepancy_test_type == DiscrepancyTestType.KCI:
            for (c1, c2) in list(combinations(contexts, 2)):
                ...
                for regime in set(regimes):
                    ...
                    data_pa_i, data_all, data_node_i = collect_subsample (datasets, target_links, c1, regime, contexts, regimes,
                                                                                node, windows_T)
                    gp_time_space[c1][regime] = (None, None, data_pa_i, data_node_i, data_all)
                    data_pa_j, data_all, data_node_j = collect_subsample (datasets, target_links, c2, regime, contexts, regimes,
                                                                                node, windows_T)
            ...
            weights = {c :  np.mean(np.array([fit_functional_model_r_c(c, r, TimeseriesScoringFunction.GP ,  datasets, windows_T,
                                         target_links, node, contexts, regimes, False)[0] for r in set(regimes)]))
                       for c in set(contexts) }

            contexts_per_node[node] = [0] if len(weights)  == 1 else   _cluster_weights(weights)[0]

    return contexts_per_node


def known_cps(datasets, links, params, options, skip, windows_T, discrepancy_test):
    res = partition_regimes(datasets, links, windows_T, params, discrepancy_test)
    regimes = regimes_map_from_constraints(res)
...
def partition_regimes(datasets, links, windows_T, params, discrepancy_test_type): #.KCI):
    """
    ...
    for node in range(params['N']):
        ...
        if discrepancy_test_type == DiscrepancyTestType.MDL:
            ...
            for w in ws:
                data_pa_i, data_all, data_node_i =  collect_subsample(datasets, target_links, 0, w, cs, ws, node, windows_T)
...
def _mdl_error_cp_search_seq(datasets, links, params, options, skip, discrepancy_test, min_dur=10, k=5, alpha=0.005):
    cuts = [0]
...
def get_error(datasets, links, gps, contexts, skip, params):
    ...
    for node in gps.keys():
        ...
        for c in set(contexts[node]):
            ...
            data_pa_i, data_all, data_node_i =  collect_subsample(
