...
class Func(Enum):
    ...
    @staticmethod
    def id_to_func(id):
        return [Func.LIN, Func.NLIN, Func.TANH, Func.SINC][id]
...
class Noise(Enum):
    ...
    @staticmethod
    def id_to_noise(id):
        return [Noise.GAUSS, Noise.UNIFORM][id]
    ...
    for r in range(params['R']):
        ...
        for t in intervention_targets_regimes[r]:
            if hard_intervention:
                ...
                w = unif_away_zero()[0]
                iter = 0
                while abs(w - initial_weights.arc_weights[t]) < strength and iter < 1000:
                    iter += 1
                    w = unif_away_zero()[0]
                r_weights[t] = w
                ...
                if arc in intervention_targets_regimes[r] and arc in intervention_targets[c]:
                    if hard_intervention:
                        ...
                        while abs(w - initial_weights.arc_weights[arc]) < strength and abs(
                                w - r_weights[arc]) < strength and iter < 1000:
                            iter += 1
                            w = unif_away_zero()[0]
