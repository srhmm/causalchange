...
from experiments.river.config import RiverLoadConfig, default_river_spacetime_config, river_reference_graph
from experiments.river.load import load_river_data


def run_river_discovery(
    *,
    ...
    config = default_river_spacetime_config(score_type=score_type)
    graph = river_reference_graph(tau_max=config.tau_max) if use_reference_graph_for_posthoc else None

    run = fit_spacetime(
        river_data.selected_year,
        config=config,
    ...
    out_dir = out_dir or results_dir("river", "discovery", create=True)
